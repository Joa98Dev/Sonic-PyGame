...
pygame.mixer.music.load("music/game_over_theme.mp3")
...
# Menu options
start_text = font.render("PRESS ENTER to Continue", True, WHITE)
quit_text = font.render("PRESS ESC to Main Menu", True, WHITE)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_ESCAPE:  # Quit game on ESC key
                pygame.quit()
                subprocess.run(["python3", "menu.py"])  # Go to main menu
                sys.exit()
